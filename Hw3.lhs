...
â€œHW3â€; you will receive a confirmation email after submitting. 
...
>  if k < k'  then 
>    let t = bstInsert k v l
>    in if height t - height r > 2 then
>       case t of Bind k1 v1 l1 r1 -> if k <= k1
>                                          then Bind k1 v1 l1 (Bind k' v' r1 r)
>                                     else
>                                          case r1 of Bind k2 v2 l2 r2 -> Bind k2 v2 (Bind k1 v1 l1 l2) (Bind k' v' r2 r)
>       else Bind k' v' t r
>  if k > k'  then 
>    let t = bstInsert k v r
>    in if height t - height l > 2 then
>       case t of Bind k1 v1 l1 r1 -> if k >= k1
>                                          then Bind k1 v1 (Bind k' v' l l1) r1
>                                     else
>                                          case l1 of Bind k2 v2 l2 r2 -> Bind k2 v2 (Bind k' v' l l2) (Bind k1 v1 r2 r1)
>       else Bind k' v' l t
>  if k == k' then Bind k v l r
...
>  | k > k' = 
>    let t = bstDelete k r
>    in if height l - height t > 2 then
>       case l of Bind k1 v1 l1 r1 -> if height l1 >= height r1
>                                           then Bind k1 v1 l1 (Bind k' v' r1 t)
>                                     else
>                                           case r1 of Bind k2 v2 l2 r2 -> Bind k2 v2 (Bind k1 v1 l1 l2) (Bind k' v' r2 t)
>       else Bind k' v' l t
>  | k < k' = 
>    let t = bstDelete k l
>    in if height r - height t > 2 then
>       case r of Bind k1 v1 l1 r1 -> if height r1 >= height l1
>                                           then Bind k1 v1 (Bind k' v' t l1) r1
>                                     else
>                                           case l1 of Bind k2 v2 l2 r2 -> Bind k2 v2 (Bind k' v' t l2) (Bind k1 v1 r2 r1)
>       else Bind k' v' t r
>  | k == k' = 
>    if height l == 0 && height r == 0 
>    then Emp
>    else if height l > height r 
>    then case l of Bind k1 v1 l1 r1 -> if height l1 >= height r1
>                                       then Bind k1 v1 l1 (bstDelete k (Bind k' v' r1 r))
>                                       else
>                                            case r1 of Bind k2 v2 l2 r2 -> Bind k2 v2 (Bind k1 v1 l1 l2) (bstDelete k (Bind k' v' r2 r))
>    else case r of Bind k1 v1 l1 r1 -> if height l1 <= height r1
>                                       then Bind k1 v1 (bstDelete k (Bind k' v' l l1)) r1
>                                       else
>                                            case l1 of Bind k2 v2 l2 r2 -> Bind k2 v2 (bstDelete k (Bind k' v' l l2)) (Bind k1 v1 r2 r1)
...
1. Using `prop_bitAdder_Correct` as a model, write a speciï¬cation for a
...
>   | (binary xs) == 0 = (binary (sampleN out), binary [sample1 cout]) == (0, binary (sample1 cin))
>   |  otherwise = binary (sampleN out ++ [sample1 cout]) == binary xs - binary (sample1 cin)
>     where (out, cout) = bitSubtractor (cin, map lift0 xs)

2. Using the `bitAdder` circuit as a model, deï¬ne a `bitSubtractor` 
circuit that implements this functionality and use QC to check that 
your behaves correctly.

> subHelper :: (Signal, [Signal]) -> ([Signal], Signal)
> subHelper (cin, [])   = ([], cin)
> subHelper (cin, x:xs) = (diff:diffs, cout)
>   where (diff,  c)  = (xor2 (x, cin), and2 (xor2 (x, high), cin)) 
>         (diffs, cout) = subHelper (c, xs)

> bitSubtractor :: (Signal, [Signal]) -> ([Signal], Signal)
> bitSubtractor (bit, xs) = (diff, cout)
>   where (diffs, cout) = subHelper (bit, xs)
>         (_, diff) = unzip (map (\val -> demux (val, cout)) diffs)


Problem: Multiplication
-----------------------

3. Using `prop_Adder_Correct` as a model, write down a QC speciï¬cation 
...
>   binary (sampleN val) == binary x * binary y
>   where val = multiplier (map lift0 x, map lift0 y)

4. Deï¬ne a `multiplier` circuit and check that it satisï¬es your 
speciï¬cation. (Looking at how adder is deï¬ned will help with this, 
but youâ€™ll need a little more wiring. To get an idea of how the 
...
> multiplier (_,[]) = []
> multiplier ([],_) = []
> multiplier (xs, y:[]) = bitMul y xs
> multiplier (xs, y:ys) = adder((bitMul y xs), multiplier(low:xs, ys))

> bitMul :: Signal->[Signal]->[Signal]
> bitMul _ [] = []
> bitMul y (x:xs) = (lift3 (\branch x y -> if branch then x else y) (y, x, low)) : (bitMul y xs)   
